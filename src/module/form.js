...
class EditComponent extends React.Component{
  ...
  sendSave(){
    ...
    if (this.state.selectRole==0) {
      alert("Phai chon quyen")
    }
    else if (this.state.campPhone=="") {
       alert("Phai them phone")
    }
    else if (this.state.campName=="") {
       alert("Phai them name")
    }
    else if (this.state.campEmail=="") {
       alert("Phai them mail")
    }
    else if (this.state.campAddress=="") {
       alert("Phai them dia chi")
    }
    else {
 
      const baseUrl = "/employee/create";
      ...
      .then(response=>{
        if (response.data.success===true) {
          alert("fawefaweddd===="+response.data.message);
          const history = useHistory();
          history.goBack();
        }
        else {
          alert("dddddd "+response.data.message)
        }
      }).catch(error=>{
